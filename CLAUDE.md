...
| ORM            | **Drizzle ORM 0.45 + drizzle-kit + node-postgres + @paralleldrive/cuid2**      | Prisma 7 + `@prisma/adapter-pg` 가 prod VM에서 P2028 ("Transaction not found") regression — interactive transaction 못 씀. Drizzle은 `BEGIN/COMMIT`을 `pg.Client` 위에 직접 발행, 어댑터 추상화 0 → trans 신뢰성 회복. **2026-04-30 마이그레이션 완료** (9 PR, 175 + 5 green) |
...
- **Drizzle 패턴** — schema는 `packages/database/src/schema/*.ts`에 hand-written (table + relations per file). `drizzle-kit generate`가 `packages/database/drizzle/NNNN_*.sql` migration 파일 생성. runtime client는 `dbDrizzle` (node-pg pool 위에 lazy Proxy, Next.js build-time eval 안전). `cuid2` (`@paralleldrive/cuid2`)가 `$defaultFn`으로 ID 생성. 모든 caller는 `import { dbDrizzle, schema, eq, and, ... } from '@hearth/database'`만. 직접 `drizzle-orm/*` import 금지.
- **runMigrations + adoption path** — `apps/bot/src/index.ts`가 부팅 시 `runMigrations(env.DATABASE_URL)` 호출 (실패 시 process.exit(1)). `packages/database/src/migrate.ts`가 fresh DB와 prisma-managed prod DB 양쪽 처리: `public.Panel` 존재 + `drizzle.__drizzle_migrations` 미존재 시 0000_init hash를 사전 등록 → migrator가 0000 skip. **검증**: prisma migration 적용된 컨테이너에 runMigrations 실행 → pg_dump diff = `drizzle.__drizzle_migrations` 테이블만 추가, 비즈니스 schema 0 변경.
- **Drizzle transaction 신뢰성** — `db.transaction(async (tx) => { ... })`이 `BEGIN/COMMIT`을 `pg.Client`에 직접 발행 (driver-adapter 추상화 0). Prisma 7에서 발생한 P2028 condition 미발생. `openTicket`은 `withAdvisoryLock`(packages/tickets-core/src/lib/advisoryLock.ts) 안에서 ticket+event를 같은 tx로 INSERT, Discord 채널-create는 lock 밖에서 (slow REST가 lock 안 잡게). orphan 채널 rollback 경로 영구 제거.
...
│       └── package.json
├── packages/
│   ├── database/                  # Drizzle schema + 마이그레이션 + 클라이언트 export
│   │   ├── drizzle/               # NNNN_*.sql migration 파일 (drizzle-kit generate 출력)
│   │   ├── drizzle.config.ts      # drizzle-kit 설정 (schema location, dialect, out)
│   │   └── src/
│   │       ├── client.drizzle.ts  # Proxy 기반 lazy NodePgDatabase (Next.js build-time eval 안전)
│   │       ├── migrate.ts         # runMigrations() — bot 부팅 시 호출. adoption path 포함
│   │       ├── errors.ts          # isUniqueViolation/isLockNotAvailable/isForeignKeyViolation (pg SQLSTATE 헬퍼)
│   │       ├── types.ts           # InferSelectModel 기반 row types + TicketStatus const+type
│   │       ├── schema/            # _enums.ts + 5 table files + index.ts (relations 포함)
│   │       └── index.ts           # 공개 surface (dbDrizzle, schema, model types, ops re-export)
...
- **활성 라우트(navigation 하이라이트)는 client-side `usePathname()`로.** App Router는 같은 layout을 child page navigation 간 재사용 — server layout에서 `headers()` + `x-pathname`으로 활성 path를 계산해서 prop으로 넘기면 첫 진입 path에 고착됨. 단, server layout에서 client component로 데이터 prop 넘길 땐 **client component가 server-only 모듈(env, db, tickets-core 배럴)을 transitively import 안 하도록 주의** — props로 평면화해서 넘기면 webpack이 client bundle에 끌고 오지 않음.
- **DB 에러 분기는 헬퍼만 사용** — Prisma `Prisma.PrismaClientKnownRequestError && code==='P2002'` 같은 패턴 금지. `import { isUniqueViolation, isLockNotAvailable, isForeignKeyViolation, getConstraintName } from '@hearth/database'`로 분기. Drizzle은 pg 네이티브 에러를 `cause` 체인으로 감싸기 때문에 헬퍼가 wrap 풀어서 SQLSTATE (23505/55P03/23503) 검사. `ticket_open_dedupe`(alreadyOpen) vs `Ticket_channelId_key`(channel race)는 `getConstraintName(e)`로 구분.
...
- DB 클라이언트는 절대 `apps/*/src/`에 직접 두지 않는다 — `packages/database`의 `dbDrizzle` (lazy Proxy)만 사용. `drizzle-orm/*`을 caller가 직접 import하지 않는다 — `@hearth/database`가 hot operators (`eq/and/inArray/asc/desc/count/sql`) 모두 re-export.
...
데이터 모델은 `packages/database/src/schema/*.ts`에 hand-written (Drizzle pgTable). 5 테이블 (GuildConfig / Panel / PanelTicketType / Ticket / TicketEvent) + `TicketStatus` pgEnum. 핵심 invariant — `ticket_open_dedupe`가 partial unique `(guildId, openerId, panelTypeId) WHERE status IN ('open','claimed')`로 동시성 race 차단. 자세한 schema는 코드 참조.
...
- ❌ `apps/*/src/`에서 직접 `drizzle-orm/*` import — `@hearth/database`의 re-export만 사용
- ❌ `@prisma/*` 패키지 import 또는 `db.X.findMany()` 같은 Prisma 호출 surface 사용 — Drizzle 마이그레이션 후 의존성 0
...
- Drizzle migrations (drizzle-kit) — https://orm.drizzle.team/kit-docs/overview
- Drizzle relational query API — https://orm.drizzle.team/docs/rqb
- @paralleldrive/cuid2 — https://github.com/paralleldrive/cuid2
- @electric-sql/pglite (test infra) — https://pglite.dev/
...
| #     | Commit    | 내용                                                                                                               |
| ----- | --------- | ------------------------------------------------------------------------------------------------------------------ |
| PR-1  | `10e2eb5` | drizzle scaffold (additive). schema 5 files + drizzle.config.ts + 0000_init.sql. import 0, behavior 0              |
| PR-2a | `6d7c7b8` | tickets-core services Drizzle 포팅 + fakeDb (1734 lines) → pglite (~70 lines). 97 unit green                       |
| PR-2b | `cc78e89` | bot wiring (container + index) + integration test 갱신 (testcontainers + Drizzle migrator) + docker compose 스모크 |
| PR-3  | `3776c8e` | dashboard 9 RSC + 3 Server Actions Drizzle 전환. dashboard tests pglite로 전환. db (Prisma) export 제거            |
| PR-4  | `ba4c21b` | runMigrations() at bot boot + adoption path (prisma-managed prod DB → drizzle 인계 자동, schema 변경 0)            |
| PR-5  | `c9b1be5` | Dockerfile entrypoint에서 `prisma migrate deploy` 제거. ENTRYPOINT = `tini -- node dist/index.js`                  |
...
- ✅ **Phase 0**: 모노레포 스캐폴딩 + Sapphire bootstrap + multi-entry tsup + 전 quality gate. 함정 16건: vault `03_troubleshooting/01-phase-0-gotchas.md`

- ✅ **Phase 0 (legacy, 별도 PR sequence)**: 모노레포 스캐폴딩 + Sapphire bootstrap + multi-entry tsup + 전 quality gate. FanX `Kayen Test Sever`에서 `/ping` 응답. 함정 16건: vault `03_troubleshooting/01-phase-0-gotchas.md`
- ✅ **Phase 1 (legacy)**: 5 PR + post-Phase-1 fix 7 commits. 티켓 lifecycle E2E 검증. 함정 9건: vault `03_troubleshooting/02-phase-1-gotchas.md`
- ✅ **PR-6 (legacy, Multi-type panel)**: 1 panel = N 버튼. `/panel ticket-type {add, edit, remove}` operator-driven CRUD
- ✅ **Phase 2 (legacy): Web Dashboard MVP**: bot + 대시보드가 같은 VM에 단일 compose stack으로. Path C (self-host, Vercel/Neon X) 채택.
  - PR-1: `@hearth/tickets-core` 추출 — 양쪽이 같은 services + zod schemas + i18n 사용. discord-api-types만, discord.js runtime 의존 0
  - PR-2: 봇 internal HTTP API — `/healthz`, `/internal/guilds/list`, `/internal/guilds/:id/resources`, `POST /internal/panels/:id/render`, `DELETE /internal/panels/:id`, `POST /internal/resolve` (snowflake → username/channel name). bearer auth (timingSafeEqual)
...
- ✅ **로컬 docker 검증** (2026-04-28): 시크릿 창 OAuth → guild picker → panel/type/settings CRUD. 여섯 함정 처리 — Auth.js의 `useSecureCookies`가 NODE_ENV=production + http에서 `__Secure-` 쿠키 prefix 박아 PKCE 깨짐 → URL scheme 기반으로 분기 (`fix(core): scope Auth.js secure cookies to https NEXTAUTH_URL`). 폼 검증이 서버에 가서야 실패하는 UX → react-hook-form + zodResolver로 client-side onChange 검증, schemas는 `@hearth/tickets-core/schemas` subpath로 import (배럴은 pg/dns/net 끌고 와서 webpack 폭발). Server Action이 ConflictError 같은 Error class 인스턴스를 Result에 담아 반환하면 React Flight가 `$Z` placeholder로 redact → 사용자에게 generic "render error" 표시. plain `ActionError { code, message }` 반환으로 전환 (`fix(core): return plain ActionError from Server Actions`). Discord 일시 실패에 fetchUserGuilds가 `[]` 반환 → layout이 `/select-guild`로 redirect → Next.js 15 router cache가 그 redirect 캐시 → reload 전엔 안 풀림. last-known-good 캐시로 폴백 (`fix(core): keep stale guild cache when Discord OAuth call fails`). Sidebar가 server layout에서 path 받아 그렸는데 App Router가 같은 layout을 child page navigation에 재사용 → sidebar 활성 표시가 stale → Sidebar를 client component로 전환 + `usePathname()` (`fix(core): sidebar highlight follows client-side navigation`). 티켓 리스트/상세에 raw snowflake 노출 → 봇이 `POST /internal/resolve`로 cache + REST fallback 통해 `@username` / `#channel-name` 리졸브 (`feat(tickets): resolve opener + channel snowflakes to display names`)
- ✅ **테스트 누계**: bot 28 unit + tickets-core 94 unit + dashboard 40 unit + bot 5 integration. 전부 green
- ✅ **첫 production VM 배포** (2026-04-29): `community-bot.namusunmul.com` (Daniel 본인 도메인 + GCP e2 VM). 새 Discord application (`Sunmul Bot`) 등록 → OS 셋업 (Ubuntu + Docker + nginx + certbot) → Cloudflare DNS A → repo clone `/sw/hearth` → env 작성 (시크릿 자동 생성) → nginx + TLS → `./infra/deploy.sh` → bot/dashboard/postgres healthy → 브라우저 OAuth → ticket #3 정상 생성. 배포 중 함정 처리 — GCP firewall :80/:443 inbound, Cloudflare proxy 모드 OFF, sites-enabled/ 디렉토리 부재 케이스, Bash `\\` 줄바꿈 trailing space, Discord Portal redirect URI 저장 검증, Privileged Gateway Intents (Server Members) 활성화. **추가 white-label sweep**: nginx example rename (`bot-dashboard.fanx.xyz.conf.example` → `dashboard.conf.example`), 두 .env.example의 `BOT_NAME=Fannie` → `Hearth`, runbook + README 모든 도메인 placeholder. **신규 함정 — Prisma 7 driver-adapter (`@prisma/adapter-pg`)의 interactive transaction이 production VM에서 P2028 발생** → workaround: `openTicket`의 `$transaction(async fn)` 제거, ticket+event 별개 INSERT (event는 best-effort try/catch). 7번째 함정으로 vault `03-phase-2-gotchas.md` 등록. **Technical debt**: native Prisma engine 회귀 (driver-adapter 제거) Phase 3 시작 전 검토
- 🚧 **PM 검증**: PM Discord 계정으로 `https://community-bot.namusunmul.com` 접근 → OAuth → 본인 길드 (Manage Guild 권한) 보이고 dashboard CRUD 가능. 봇이 PM 길드에 invite돼야 picker에 떠서 작업 가능
- 🚧 **회사 도메인 + 팀 GCP 배포**: namusunmul 배포에서 검증된 흐름 그대로 — `bot-dashboard.fanx.xyz` 또는 팀이 정한 서브도메인. 봇 토큰은 새 application 또는 기존 Fannie 이전
