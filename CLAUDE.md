...
- Obsidian vault `/Users/ahn_euijin/Desktop/01_vault`에 프로젝트 문서 살림. **vault 폴더는 `10_study-with-agents/02_projects/hearth/`** 로 rename 예정 (`discord-bot/` → `hearth/`). 메모리 폴더 (`~/.claude/projects/-Users-ahn-euijin-discord-bot/`)는 로컬 repo 경로에서 자동 파생 — 로컬 dir 이름 (`/Users/ahn_euijin/discord-bot/`) 바꾸지 않는 한 그대로.
