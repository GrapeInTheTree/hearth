...
| 도메인 패키지  | **`@discord-bot/tickets-core`** (티켓 도메인 services + ports + i18n + zod schemas) | 봇/대시보드 양쪽이 import. discord-api-types만 사용 (런타임 의존 0)                                   |
...
│   │   │   ├── lib/               # 봇-only 유틸 (interactionHelpers, replyEphemeral, logger 등). 도메인 유틸은 @discord-bot/tickets-core에
...
└── docker-compose
    ...
    # redis는 Phase 4에서 추가 (이미지 슬롯 discord-bot-cache:local 예약됨)
```
...
- ✅ **Phase 2: Web Dashboard MVP (PR-1~7)**: bot + 대시보드가 같은 VM에 단일 compose stack으로. Path C (self-host, Vercel/Neon X) 채택.
  - PR-1: `@discord-bot/tickets-core` 추출 — 양쪽이 같은 services + zod schemas + i18n 사용. discord-api-types만, discord.js runtime 의존 0
  ...
  - PR-7: dashboard Dockerfile + compose 갱신 (image rename `discord-bot-bot:local` + `discord-bot-dashboard:local`, env-driven 호스트 포트), nginx 예시, deploy.sh `INTERNAL_API_TOKEN` 일치 검증, runbook
