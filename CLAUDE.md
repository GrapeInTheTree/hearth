...
- **Prisma `$transaction(async fn)` interactive transaction 사용 자제.** Prisma 7 + `@prisma/adapter-pg` driver-adapter 조합은 production VM에서 P2028 ("Transaction not found") 빈발 — transaction tracking이 신뢰성 낮음. 로컬에선 안 보이고 prod에서만 발현하는 류의 환경 의존 버그. 다중 write가 필요하면 (a) partial unique index + best-effort 패턴 (e.g. `openTicket`), (b) Prisma nested write (`{ events: { create: ... } }`), (c) raw SQL with explicit BEGIN/COMMIT 중 택. **Technical debt: 이 driver-adapter를 native engine으로 회귀하는 작업 Phase 3 시작 전 검토 — 그러면 다시 `$transaction(async)` 신뢰성 회복 가능.**
