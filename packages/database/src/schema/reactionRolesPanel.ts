...
// A reaction-roles panel — one Discord message that surfaces N emoji reactions.
// Each reaction is bound to its own role (per-option roleId on
// ReactionRolesOption). Adding a reaction grants the option's role; removing
...
export const reactionRolesPanel = pgTable(
  'ReactionRolesPanel',
  {
    id: text('id')
      .primaryKey()
  ...
  (t) => [
    index('ReactionRolesPanel_guildId_idx').on(t.guildId),
    ...
    index('ReactionRolesPanel_messageId_idx').on(t.messageId),
    uniqueIndex('ReactionRolesPanel_guildId_channelId_messageId_key').on(
      t.guildId,
      t.channelId,
      t.messageId,
    ),
...
export const reactionRolesPanelRelations = relations(reactionRolesPanel, ({ many }) => ({
  options: many(reactionRolesOption),
}));
