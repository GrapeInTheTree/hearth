...
// One emoji-to-role binding on a self-roles panel. Adding the reaction
...
export const selfRolesOption = pgTable(
  'SelfRolesOption',
  {
    id: text('id')
      .primaryKey()
  ...
  (t) => [
    index('SelfRolesOption_panelId_idx').on(t.panelId),
    ...
    uniqueIndex('SelfRolesOption_panelId_emoji_key').on(t.panelId, t.emoji),
    uniqueIndex('SelfRolesOption_panelId_position_key').on(t.panelId, t.position),
    uniqueIndex('SelfRolesOption_panelId_label_key').on(t.panelId, t.label),
  ],
);

export const selfRolesOptionRelations = relations(selfRolesOption, ({ one }) => ({
  panel: one(selfRolesPanel, {
    fields: [selfRolesOption.panelId],
