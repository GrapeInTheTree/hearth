...
interface NewXWatcherPageProps {
  readonly params: Promise<{ readonly guildId: string }>;
}

export default async function NewXWatcherPage({
  params,
}: NewXWatcherPageProps): Promise<React.JSX.Element> {
  ...
  return (
    <>
      <Topbar
        ...
        avatarUrl={avatarUrl}
        title="New watcher"
        description="Pick the X account to follow and the channel its posts should land in."
      />
      <main className="mx-auto w-full max-w-5xl flex-1 px-8 py-12">
        {!resources.ok ? (
          ...
          <XWatcherForm guildId={guildId} channels={resources.value.channels} />
        )}
      </main>
    </>
  );
