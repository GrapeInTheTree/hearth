...
import { ReactionRolesPanelForm } from '@/components/reaction-roles/reaction-roles-panel-form';
import { Card, CardContent } from '@/components/ui/card';
import { auth } from '@/lib/auth';
import { callBot } from '@/lib/botClient';
import type { GuildResources } from '@/types/bot';

interface NewReactionRolesPageProps {
  readonly params: Promise<{ readonly guildId: string }>;
}

export default async function NewReactionRolesPage({
  params,
}: NewReactionRolesPageProps): Promise<React.JSX.Element> {
  ...
  return (
    <>
      <Topbar
        ...
        avatarUrl={avatarUrl}
        title="New reaction-roles panel"
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-8 py-12">
        {!resources.ok ? (
          ...
          <ReactionRolesPanelForm guildId={guildId} channels={resources.value.channels} />
        )}
      </main>
    </>
  );
