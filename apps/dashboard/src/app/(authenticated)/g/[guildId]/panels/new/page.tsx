...
interface NewPanelPageProps {
  readonly params: Promise<{ readonly guildId: string }>;
}

export default async function NewPanelPage({
  params,
}: NewPanelPageProps): Promise<React.JSX.Element> {
  ...
  return (
    <>
      <Topbar
        ...
        avatarUrl={avatarUrl}
        title="New panel"
        description="Pick a channel, write the embed copy. Ticket types are added next."
      />
      <main className="mx-auto w-full max-w-5xl flex-1 px-8 py-12">
        {!resources.ok ? (
          ...
          <PanelForm guildId={guildId} channels={resources.value.channels} />
        )}
      </main>
    </>
  );
