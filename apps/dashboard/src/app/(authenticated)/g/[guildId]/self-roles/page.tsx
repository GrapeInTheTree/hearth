...
interface SelfRolesListPageProps {
  readonly params: Promise<{ readonly guildId: string }>;
}

export default async function SelfRolesListPage({
  params,
}: SelfRolesListPageProps): Promise<React.JSX.Element> {
  const session = await auth();
  if (session === null) redirect('/login');

  const { guildId } = await params;
  const panels = await dbDrizzle.query.selfRolesPanel.findMany({
  ...
  return (
    <>
      <Topbar
        ...
        action={
          <Button asChild>
            <Link href={`/g/${guildId}/self-roles/new`}>New panel</Link>
          </Button>
        }
      />
        ...
        {panels.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
              <Languages
                ...
                aria-hidden="true"
              />
              <p className="text-base font-medium">No self-roles panels yet</p>
              ...
              <Button asChild>
                <Link href={`/g/${guildId}/self-roles/new`}>Create your first panel</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
          <ul className="grid gap-3">
            {panels.map((p) => {
              ...
              return (
                <li key={p.id}>
                  <Link href={`/g/${guildId}/self-roles/${p.id}`} className="block">
