...
import { DeleteSelfRolesButton } from '@/components/self-roles/delete-self-roles-button';
import { RemoveSelfRolesOptionButton } from '@/components/self-roles/remove-self-roles-option-button';
import { RepostSelfRolesButton } from '@/components/self-roles/repost-self-roles-button';
import { RetrySyncSelfRolesButton } from '@/components/self-roles/retry-sync-self-roles-button';
import { SelfRolesPreview } from '@/components/self-roles/self-roles-preview';
...
interface SelfRolesDetailPageProps {
...
export default async function SelfRolesDetailPage({
  params,
}: SelfRolesDetailPageProps): Promise<React.JSX.Element> {
  ...
  const panel = await dbDrizzle.query.selfRolesPanel.findFirst({
    ...
    with: { options: { orderBy: asc(schema.selfRolesOption.position) } },
  });
      ...
      <Topbar
        ...
        action={
          <div className="flex items-center gap-2">
            {!isStale ? <RepostSelfRolesButton guildId={guildId} panelId={panelId} /> : null}
            <Button asChild variant="secondary" size="sm">
              <Link href={`/g/${guildId}/self-roles/${panelId}/edit`}>Edit</Link>
            </Button>
          </div>
        }
      />
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-8 py-12">
        {isStale ? (
          <Card>
            ...
            <CardContent>
              <RetrySyncSelfRolesButton guildId={guildId} panelId={panelId} />
            </CardContent>
          </Card>
        ) : null}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <Button asChild variant="ghost" size="sm">
                          <Link href={`/g/${guildId}/self-roles/${panelId}/options/${o.id}/edit`}>
                            Edit
                          </Link>
                        </Button>
                        <RemoveSelfRolesOptionButton
                          guildId={guildId}
                          panelId={panelId}
                          optionId={o.id}
                          optionLabel={o.label}
        ...
        <Card destructive>
          ...
          <CardContent>
            <DeleteSelfRolesButton guildId={guildId} panelId={panelId} />
          </CardContent>
        </Card>
      </main>
    </>
