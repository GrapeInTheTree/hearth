...
import { SelfRolesPanelInputSchema } from '@hearth/self-roles-core/schemas';
...
import { SelfRolesPreview } from './self-roles-preview';

import { createSelfRolesPanel, updateSelfRolesPanel } from '@/actions/self-roles';
...
interface SelfRolesPanelFormProps {
...
const SESSION_KEY_PREFIX = 'self-roles-panel-form:';

const FormSchema = SelfRolesPanelInputSchema.extend({
...
export function SelfRolesPanelForm({
  guildId,
  channels,
  initial,
}: SelfRolesPanelFormProps): React.JSX.Element {
  ...
  async function onSubmit(values: FormValues): Promise<void> {
    try {
      ...
      router.push(`/g/${guildId}/self-roles/${result.value.value.panelId}`);
      router.refresh();
    } catch (e) {
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="grid gap-2">
          <Label htmlFor="self-roles-channel">Channel</Label>
          <Controller
            name="channelId"
            control={control}
            render={({ field }) => (
              <ChannelPicker
                id="self-roles-channel"
                channels={channels}
                value={field.value}
          ...
          {errors.channelId !== undefined ? (
            ...
            <p className="text-xs text-[color:var(--color-fg-muted)]">
              The self-roles message will be posted here. Languages typically live in a #info or
              #welcome channel.
            </p>
          )}
        </div>
        ...
        <div className="grid gap-2">
          <Label htmlFor="self-roles-title">Embed title</Label>
          <Input
            id="self-roles-title"
            maxLength={256}
        ...
        <div className="grid gap-2">
          <Label htmlFor="self-roles-description">Embed description</Label>
          <Textarea
            id="self-roles-description"
            maxLength={4000}
      ...
      <div className="lg:sticky lg:top-20 lg:self-start">
        <SelfRolesPreview title={title} description={description} />
      </div>
    </form>
  );
}
