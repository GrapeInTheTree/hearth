...
import { SelfRolesOptionInputSchema } from '@hearth/self-roles-core/schemas';
...
import { addSelfRolesOption, updateSelfRolesOption } from '@/actions/self-roles-options';
import { RolePicker } from '@/components/pickers/role-picker';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';

const FormSchema = SelfRolesOptionInputSchema;
...
interface SelfRolesOptionFormProps {
...
export function SelfRolesOptionForm({
  guildId,
  panelId,
  roles,
  initial,
}: SelfRolesOptionFormProps): React.JSX.Element {
  ...
  async function onSubmit(values: FormValues): Promise<void> {
    try {
      const result =
        initial !== undefined
          ? await updateSelfRolesOption({
              guildId,
              panelId,
              optionId: initial.optionId,
              input: values,
            })
          : await addSelfRolesOption({ guildId, panelId, input: values });
      ...
      router.push(`/g/${guildId}/self-roles/${panelId}`);
      router.refresh();
    } catch (e) {
