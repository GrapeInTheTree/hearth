...
import {
  createSelfRolesPanel,
  deleteSelfRolesPanel,
  repostSelfRolesPanel,
  updateSelfRolesPanel,
} from '@/actions/self-roles';
...
describe('createSelfRolesPanel', () => {
  ...
  it('inserts a panel row with placeholder messageId', async () => {
    const result = await createSelfRolesPanel({
      guildId,
      input: {
        guildId,
        channelId,
    ...
    const rows = await testDb.db
      ...
      .where(eq(schema.selfRolesPanel.id, result.value.value.panelId));
...
describe('updateSelfRolesPanel', () => {
  ...
  it('updates only the provided fields and triggers a render call', async () => {
    ...
    const result = await updateSelfRolesPanel({
      guildId,
      panelId,
      channelId: undefined,
