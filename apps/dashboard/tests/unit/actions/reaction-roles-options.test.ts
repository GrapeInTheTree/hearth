...
describe('addReactionRolesOption', () => {
  ...
  it('adds a second option with a different emoji and auto-syncs to Discord', async () => {
    const result = await addReactionRolesOption({
      guildId,
      panelId,
      input: {
        label: 'Korean',
...
describe('updateReactionRolesOption', () => {
  let testDb: DashboardTestDb;
  let panelId: string;
  let optionId: string;
  ...
  it('updates only the supplied fields', async () => {
    const result = await updateReactionRolesOption({
      guildId,
      panelId,
      optionId,
...
describe('removeReactionRolesOption', () => {
  let testDb: DashboardTestDb;
  let panelId: string;
  let optionId: string;
  ...
  it('deletes the option', async () => {
    const result = await removeReactionRolesOption({ guildId, panelId, optionId });
    expect(result.ok).toBe(true);
    const rows = await testDb.db
      .select()
      .from(schema.reactionRolesOption)
